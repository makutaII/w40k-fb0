<template>
  <v-container>
    ...
    <v-row>
      <v-col cols="12" v-for="(char, с) in charsD">
        <v-sheet class="mt-5 pa-6" elevation="2">
          ...
          <h4 class="ma-2">Умения</h4>
          <v-sheet
            style="min-width: 300px"
            v-for="(skill, i) in char.skills"
            :key="i"
            class="ma-2"
          >
            <v-card>
              ...
              >
                ...
                <v-btn
                  :icon="
                    showId == skill.id ? 'mdi-chevron-up' : 'mdi-chevron-down'
                  "
                  size="small"
                  :id="skill.id"
...
async function getChars() {
  let cf = await $fetch("/api/w_characteristic/full");
  //console.log("cf=", cf);
  return cf;
}
